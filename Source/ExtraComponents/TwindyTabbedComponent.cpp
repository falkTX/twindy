...
void TwindyTabbedComponent::resized()
{
...
#if JUCE_NEW
    Rectangle<int> mainArea, outlinePos;
#else
    ...
    {
#if JUCE_NEW
        Rectangle<int> selTabBounds (selectedTab->getBounds());
#else
...
void TwindyTabbedComponentBasicTab::mouseWheelMove (const MouseEvent& e, float wheelIncrementX, float wheelIncrementY)
{
    const TwindyTabbedComponent* const t = (const TwindyTabbedComponent*)(getParentComponent()->getParentComponent());
    ...
    {
#if JUCE_NEW
        MouseWheelDetails details = { wheelIncrementX, wheelIncrementY, false, false };
        t->scrollbar->mouseWheelMove (e, details);
#else
