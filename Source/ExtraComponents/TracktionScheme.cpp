// TracktionScheme - Helper class for reading from/writing to .tracktionschemes.
// ----------------------------------------------------------------------------
...
Colour TracktionScheme::getColour(const String& name) const
{
    ...
    if (rootElement)
    {
        ...
        {
            for (XmlElement* tempElement = rootElement->getChildByName(T("COLOUR")); tempElement; tempElement = tempElement->getNextElement())
            {
                if (tempElement->getStringAttribute(T("name")) == name)
                {
                    String tempString = tempElement->getStringAttribute(T("colour"));
                    retval = Colour(tempString.getHexValue32());
                    break;
                }
            }
        }
    ...
    if (rootElement)
    {
        if (rootElement->hasTagName(T("COLOURS")))
        {
            for (XmlElement* tempElement = rootElement->getChildByName(T("COLOUR")); tempElement; tempElement = tempElement->getNextElement())
            {
                if (tempElement->getStringAttribute(T("name")) == name)
                {
                    tempElement->setAttribute(T("colour"), String::toHexString((int)newColour.getARGB()));
                    break;
                }
            }
